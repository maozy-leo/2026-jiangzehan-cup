...
def radius_token_for_analysis(radius: float) -> str:
    """
    为兼容 attack_utils.parse_attack_metadata() 当前的命名解析规则，
    这里将 radius 编码为文件名末尾的整数。
    """
...
def run_betweenness_attack_until_target_y(
    input_graph: nx.Graph,
    ...
    if not (0 <= target_y_ratio <= 1):
        raise ValueError("target_y_ratio must be between 0 and 1.")
    if sample_k <= 0:
        raise ValueError("sample_k must be positive.")
    if jump_step <= 0:
        raise ValueError("jump_step must be positive.")
    ...
    limit = working_graph.number_of_nodes() if max_attack_steps is None else min(max_attack_steps, working_graph.number_of_nodes())
    ...
    for step in range(limit):
        ...
        current_y = largest_component_ratio(working_graph, n_original)
        if current_y <= target_y_ratio:
            break
...
def choose_edges_to_add(
    ...
    candidate_edges = find_component_pair_shortest_edges_kdtree(
    ...
    for distance, u, v, ci, cj in candidate_edges:
...
# =========================================================
# Window runner (profit / gain-per-edge criterion)
# =========================================================
@dataclass
class WindowResult:
    window_index: int
    target_y_threshold: float
    baseline_attack_steps: int
...
def run_single_window(
    ...
    baseline_run = run_betweenness_attack_for_steps(
        input_graph=window_initial_graph,
    ...
    target_y_threshold = baseline_run.y_end

    accepted_start_graph = window_initial_graph.copy()
    ...
    round_logs: List[Dict[str, Any]] = [
        {
            "round_index": 0,
            "mode": "baseline",
            "target_y_threshold": target_y_threshold,
            "attack_steps": baseline_run.attack_steps,
            "y_start": baseline_run.y_start,
            "y_end": baseline_run.y_end,
            "new_edges_added": 0,
            "new_edge_total_length": 0.0,
            "incremental_attack_step_gain": 0,
            "gain_per_edge": None,
            "accepted": True,
        }
    ]
    ...
    if verbose:
        print(
            f"[window {window_index}] baseline: "
            f"baseline_steps={baseline_run.attack_steps}, target_y={target_y_threshold:.6f}"
        )

    if baseline_run.attack_steps == 0:
        return WindowResult(
            window_index=window_index,
            target_y_threshold=target_y_threshold,
            baseline_attack_steps=baseline_run.attack_steps,
            accepted_attack_sequence=baseline_run.attack_sequence,
            accepted_attack_steps=baseline_run.attack_steps,
            accepted_after_graph=baseline_run.graph_after,
            accepted_added_edges=accepted_added_edges,
            round_logs=round_logs,
        )

    for repair_round in range(1, max_repair_rounds + 1):
        components = get_split_components(
            pre_window_lcc_nodes=accepted_run.pre_window_lcc_nodes,
            after_graph=accepted_run.graph_after,
        )

        if len(components) <= 1:
            break

        proposed_edges = choose_edges_to_add(
            current_graph=accepted_run.graph_after,
            components=components,
            coords=coords,
        ...
        candidate_start_graph = accepted_start_graph.copy()
        for u, v, length in proposed_edges:
            candidate_start_graph.add_edge(
                u,
                v,
                length=float(length),
        ...
        candidate_run = run_betweenness_attack_until_target_y(
            input_graph=candidate_start_graph,
            target_y_ratio=target_y_threshold,
            n_original=n_original,
            radius=radius,
            normalized=normalized,
            sample_k=sample_k,
        ...
        incremental_attack_step_gain = candidate_run.attack_steps - accepted_run.attack_steps
        gain_per_edge = incremental_attack_step_gain / len(proposed_edges)
        accepted_this_round = gain_per_edge >= ratio_gain_threshold
        ...
        round_logs.append(
            {
                ...
                "mode": "repair",
                "target_y_threshold": target_y_threshold,
                "attack_steps": candidate_run.attack_steps,
                "y_start": candidate_run.y_start,
                "y_end": candidate_run.y_end,
                "new_edges_added": len(proposed_edges),
                "new_edge_total_length": float(sum(length for _, _, length in proposed_edges)),
                "incremental_attack_step_gain": incremental_attack_step_gain,
                "gain_per_edge": gain_per_edge,
        ...
        if verbose:
            print(
                f"[window {window_index}] repair round {repair_round}: "
                f"candidate_steps={candidate_run.attack_steps}, "
                f"incremental_gain={incremental_attack_step_gain}, "
                f"gain_per_edge={gain_per_edge:.6f}, accepted={accepted_this_round}"
            )

        if accepted_this_round:
            accepted_start_graph = candidate_start_graph
    ...
    return WindowResult(
        ...
        target_y_threshold=target_y_threshold,
        baseline_attack_steps=baseline_run.attack_steps,
        accepted_attack_sequence=accepted_run.attack_sequence,
        accepted_attack_steps=accepted_run.attack_steps,
        accepted_after_graph=accepted_run.graph_after,
        accepted_added_edges=accepted_added_edges,
        round_logs=round_logs,
    )
...
# Public interface
# =========================================================
def run_betweenness_profit_interface(
    ...
    *,
    ...
    normalized: bool = False,
    random_seed: int = 42,
    max_repair_rounds_per_window: int = 20,
    verbose: bool = False,
) -> Dict[str, str]:
    """
    接口版：对应“原始选边策略 + 新序列使用点/新增边数 比值阈值”的版本。
    ...
    - end_y_threshold
    - radius
    - add_edge_count_per_round
    - output_root

    说明：
    - 默认 window_size = jump_step
    - 若传 window_ratio，则优先按比例取窗口
    - 输出仅保留：
        1) attack sequence csv
        2) added edges csv
        3) summary json
    """
    if sample_k <= 0:
    ...
    if jump_step <= 0:
        raise ValueError("jump_step must be positive.")
    if ratio_gain_threshold < 0:
        raise ValueError("ratio_gain_threshold must be non-negative.")
    ...
    if max_repair_rounds_per_window <= 0:
        raise ValueError("max_repair_rounds_per_window must be positive.")
    if window_ratio is not None and not (0 < window_ratio <= 1):
    ...
    window_size = max(1, int(round(window_ratio * n_original))) if window_ratio is not None else jump_step
    ...
    while current_window_initial_graph.number_of_nodes() > 0:
        ...
        window_result = run_single_window(
            ...
            n_original=n_original,
            ratio_gain_threshold=ratio_gain_threshold,
            ...
            jump_step=jump_step,
            random_seed=random_seed,
            max_repair_rounds=max_repair_rounds_per_window,
            verbose=verbose,
        )
        ...
        all_window_logs.append(
            {
                ...
                "target_y_threshold": window_result.target_y_threshold,
                "baseline_attack_steps": window_result.baseline_attack_steps,
    ...
    summary_payload: Dict[str, Any] = {
        ...
        "normalized": normalized,
        "random_seed": random_seed,
        "max_repair_rounds_per_window": max_repair_rounds_per_window,
...
def main() -> None:
    outputs = run_betweenness_profit_interface(
        graph_csv_path="Chengdu_Edgelist.csv",
        sample_k=20,
        jump_step=10,
        ratio_gain_threshold=1.0,
        end_y_threshold=0.01,
        radius=0,
        add_edge_count_per_round=5,
        output_root="results_profit_interface",
        window_ratio=None,
        normalized=False,
        random_seed=42,
        max_repair_rounds_per_window=20,
        verbose=True,
    )
